...
def verify_file_format(file_bytes: bytes, declared_content_type: str) -> dict:
    ...
    if actual_format not in ALLOWED_TYPES:
        return {
            "valid":  False,
            "reason": f"Format '{actual_format}' not allowed. Only JPEG and PNG accepted.",
    ...
    if actual_format == "jpeg" and "jpeg" not in declared and "jpg" not in declared:
        mismatch = True
    if actual_format == "png" and "png" not in declared:
        mismatch = True

    if mismatch:
        return {
            "valid":  False,
            "reason": f"File mismatch. File is {actual_format} but declared as {declared_content_type}.",
            "actual": actual_format,
        }
